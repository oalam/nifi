...
@CapabilityDescription("Sends the contents of a FlowFile as a message to Apache Kafka. The messages to send may be individual FlowFiles or may be delimited, using a "
        + "user-specified regex delimiter.")
...
public class PutKafkaExt extends AbstractKafkaProcessor<KafkaPublisherExt> {
    ...
    public static final PropertyDescriptor DELIVERY_GUARANTEE = new PropertyDescriptor.Builder()
            ...
            .build();
    public static final PropertyDescriptor MESSAGE_REGEX_DELIMITER = new PropertyDescriptor.Builder()
            .name("Message Regex Delimiter")
            .description("Specifies the regex delimiter to use for splitting apart multiple messages within a single FlowFile. "
    ...
    static {
        ...
        _propertyDescriptors.add(ZOOKEEPER_CONNECTION_STRING);
        ...
        _propertyDescriptors.add(MESSAGE_REGEX_DELIMITER);
    ...
    private FlowFile doRendezvousWithKafka(final FlowFile flowFile, final ProcessContext context, final ProcessSession session) {
        ...
        session.read(flowFile, new InputStreamCallback() {
            ...
            public void process(InputStream contentStream) throws IOException {
                PublishingContextExt publishingContext = PutKafkaExt.this.buildPublishingContext(flowFile, context, contentStream);
                KafkaPublisherResult result = PutKafkaExt.this.kafkaResource.publish(publishingContext);
     ...
     */
    private PublishingContextExt buildPublishingContext(FlowFile flowFile, ProcessContext context,
        ...
        if (this.isFailedFlowFile(flowFile)) {
            ...
            delimiterRegex = flowFile.getAttribute(FAILED_DELIMITER_ATTR) != null
                    ? flowFile.getAttribute(FAILED_DELIMITER_ATTR) : null;
            ...
            delimiterRegex = context.getProperty(MESSAGE_REGEX_DELIMITER).isSet() ? context.getProperty(MESSAGE_REGEX_DELIMITER)
        ...
        publishingContext.setRegexDelimiter(delimiterRegex);
    ...
    @Override
    protected KafkaPublisherExt buildKafkaResource(ProcessContext context, ProcessSession session)
            throws ProcessException {
        KafkaPublisherExt kafkaPublisher = new KafkaPublisherExt(this.buildKafkaConfigProperties(context), this.getLogger());
    ...
    private Map<String, String> buildFailedFlowFileAttributes(int lastAckedMessageIndex, FlowFile sourceFlowFile,
        ...
        attributes.put(FAILED_DELIMITER_ATTR, context.getProperty(MESSAGE_REGEX_DELIMITER).isSet()
                ? context.getProperty(MESSAGE_REGEX_DELIMITER).evaluateAttributeExpressions(sourceFlowFile).getValue()
                : null);
        return attributes;
    }
