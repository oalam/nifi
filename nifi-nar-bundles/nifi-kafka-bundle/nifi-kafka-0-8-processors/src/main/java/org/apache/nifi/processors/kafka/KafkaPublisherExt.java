 ...
 */
class KafkaPublisherExt implements Closeable {
    ...
    KafkaPublisherExt(Properties kafkaProperties, ComponentLog componentLog) {
     ...
     *            {@link KafkaProducer}
     */
    KafkaPublisherExt(Properties kafkaProperties, int ackCheckSize, ComponentLog componentLog) {
    ...
    /**
     * Publishes messages to Kafka topic. It uses {@link StreamTokenizer} to
     ...
     * {@link StreamTokenizer#nextToken()} operation.</li>
     ...
     */
    KafkaPublisherResult publish(PublishingContextExt publishingContext) {
        StreamTokenizer streamTokenizer = new StreamTokenizer(
                publishingContext.getContentStream(),
                publishingContext.getRegexDelimiter(),
                publishingContext.getCharset());
        ...
        for (; continueSending && (messageBytes = streamTokenizer.nextToken()) != null; tokenCounter++) {
            if (prevLastAckedMessageIndex < tokenCounter) {
                ...
                ProducerRecord<byte[], byte[]> message =
                    new ProducerRecord<>(publishingContext.getTopic(), partitionId, publishingContext.getKeyBytes(), messageBytes);
